...
@section('customer')
                ...
                <div class="row mb-5">
                                ...
                                <div class="swiper product-slider-thumbs">
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide h-auto swiper-thumb-item mb-3"><img class="w-100"
                                                src="{{ Storage::url($product->image) }}" alt="..."></div>
                                        <div class="swiper-slide h-auto swiper-thumb-item mb-3"><img class="w-100"
                                                src="{{ Storage::url($product->image) }}" alt="..."></div>
                                        <div class="swiper-slide h-auto swiper-thumb-item mb-3"><img class="w-100"
                                                src="{{ Storage::url($product->image) }}" alt="..."></div>
                                    ...
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide h-auto"><a class="glightbox product-view"
                                                ...
                                                data-glightbox="Product item 1"><img class="img-fluid"
                                                    src="{{ Storage::url($product->image) }}" alt="..."></a></div>
                                        <div class="swiper-slide h-auto"><a class="glightbox product-view"
                    ...
                    <div class="col-lg-6">
                        <form id="form" method="POST">
                            ...
                            <li class="px-3 py-2 mb-1 bg-white text-muted mb-2"><strong
                                    class="text-uppercase text-dark">Price:</strong><a
                                    class="reset-anchor ms-2 text-danger" id="priceDisplay">{{ $product->price }} VNĐ</a>
                            </li>
                            {{-- color --}}
                            <li class="px-3 py-2 mb-1 bg-white text-muted mb-2"><strong
                                    class="text-uppercase text-dark">Color: </strong>
                                @php
                                    $uniqueColors = $variants->unique('color');
                                @endphp

                                @foreach ($uniqueColors as $item)
                                    <input type="radio" class="btn-check" name="color"
                                        id="color{{ $item->id }}" autocomplete="off" value="{{ $item->color }}">
                                    <label class="btn" style="color: {{ $item->color }}"
                                        for="color{{ $item->id }}">{{ $item->color }}</label>
                                @endforeach
                            </li>

                            <li class="px-3 py-2 mb-1 bg-white text-muted mb-2 mt-2">
                                <strong class="text-uppercase text-dark">Size:</strong>
                                <div id="sizeOptions" style="display: inline-block; margin-left: 10px;"></div>
                            </li>
                            ...
                            <div class="row align-items-stretch mb-4 mt-4">
                                <div class="col-sm-5 pr-sm-0">
                                    <div
                                        ...
                                        <div class="quantity">
                                            ...
                                            <input class="form-control border-0 shadow-0 p-0" id="myInput"
                                ...
                                <div class="col-sm-3 pl-sm-0">
                                    <button type="button"
                                        onclick="submitForm('{{ route('add_cart', ['idProduct' => $product->id]) }}')"
                                ...
                                <div class="col-sm-3 pl-sm-0">
                                    <button type="button"
                                        onclick="submitForm('{{ route('order', ['id' => $product->id]) }}')"
    ...
    <!-- Trong phần mã JavaScript của bạn -->
    ...
    <script>
        $(document).ready(function() {
            $('input[name="color"]').change(function() {
                ...
                var url = '/getcolor/' + productId + '/' + color;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        sizeOptions.empty(); // Xóa bỏ nội dung cũ trước khi thêm mới
                        ...
                        sizes.forEach(function(size) {
                            ...
                            var label = $('<label>').addClass('btn').attr('for',
                                'size' + size.id).text(size.size);

                            sizeOptions.append(input).append(label);
                        });
                    },
